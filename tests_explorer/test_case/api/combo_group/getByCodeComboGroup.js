...
describe("Test Suite Get By Code Combo Group", function () {
   before(async function () {
      await initial.Login_Yoshinoya();
   });

   it("Get By Code Combo Group with Valid Code and Fill all Information should be Success #20230807092500", async function () {
    let responseCode = await api_general.GetComboGroup();
    const comboGroupCode = responseCode.body[0].code; 

    const response = await chai
            .request(baseurl)
            .get('/api/combo-group/' + comboGroupCode)
   ...
   it("Get By Code Combo Group with inValid Code and Fill all Information should be Failure #20230807092500", async function () {
