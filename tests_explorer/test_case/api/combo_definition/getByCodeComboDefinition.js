...
describe("Test Suite Get By Code Combo Definition", function () {
   before(async function () {
      await initial.Login_Yoshinoya();
   });

   it("Get By Code Combo Definition with Valid Code and Fill all Information should be Success #20230807092500", async function () {
    let responseCode = await api_general.GetComboDefinition();
    const comboDefinitionCode = responseCode.body[0].code; 

    const response = await chai
            .request(baseurl)
            .get('/api/combo-definition/' + comboDefinitionCode)
    ...
    response.body.code.should.have.equal(comboDefinitionCode);
    response.body.should.have.property('name');
   });

   it("Get By Code Combo Definition with inValid Code and Fill all Information should be Failure #20230807092500", async function () {

    const response = await chai
            .request(baseurl)
            .get('/api/combo-definition/' + process.env.InvalidCode)
            .set('content-type', 'application/json')
            .set('Authorization', 'Bearer ' + global.bearerToken)
